...
export default async function Home({
  ...
  const prev = format(
    new Date(new Date(dateKey).getTime() - 24 * 60 * 60 * 1000),
    "yyyy-MM-dd"
  );
  const next = format(
    new Date(new Date(dateKey).getTime() + 24 * 60 * 60 * 1000),
          ...
          <div className="card-body">
            <div className="flex items-center justify-between mb-3">
              ...
              <nav className="flex items-center gap-3 text-sm">
                <Link href={`/?date=${prev}`} className="badge">
                  Prev
                </Link>
                <Link href={`/?date=${todayKey}`} className="badge">
                  Today
                </Link>
                <span className="text-sm font-medium">{dateKey}</span>
                <Link href={`/?date=${next}`} className="badge">
                  Next
                </Link>
              </nav>
            </div>
            ...
            <section className="space-y-2">
              ...
              >
                <input
                  name="goal"
                  type="number"
                  defaultValue={goal?.goalCalories ?? 0}
                ...
                <div key={p.id} className="flex items-center gap-2">
                  <form
                    action={async (fd) => {
                      "use server";
                      const name = String(fd.get("name") ?? "").trim();
                      const caloriesPerUnit = Number(
                        fd.get("caloriesPerUnit") ?? 0
                      );
